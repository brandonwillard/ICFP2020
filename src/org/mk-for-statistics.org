...
#+LATEX_HEADER: \affiliation{\institution{Ampersand} \city{Chicago} \state{IL} \country{USA}}
...
Monte Carlo methods, but its automation is something that falls well outside of
...
in probability theory like the following identity--expressed as a rule--about
...
[[citep:GelmanTransformingparameterssimple2019]].  Among those is the classic
...
Under the left-to-right application of Rule [[eqref:eq:normal-affine-trans]],
...
Rule [[eqref:eq:normal-affine-trans]] is useful for computational reasons (e.g.
...
Y\) in Rule [[eqref:eq:normal-affine-trans]] is a normal random variable unless
the right-to-left rule is applied.

Considerations like these lead quickly into the domain of term rewriting
[[citep:BaaderTermrewritingall1999]].  The example involving Rule
...
desire for an interactive system for ad hoc additions and experimentation.
When they are, however, we would like the means to apply methods like rewrite
rule completion.
