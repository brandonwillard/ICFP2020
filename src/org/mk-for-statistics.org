...
(add-to-list 'org-latex-classes
             '("acmart" "\\documentclass[11pt]{acmart}"
               ("\\section{%s}" . "\\section*{%s}")
...
#+BEGIN_abstract
In this article, we give a brief overview of the current state and future potential of
symbolic computation within the Python statistical modeling and machine learning
community.  We detail the use of miniKanren as an underlying framework for term
rewriting and symbolic mathematics, as well as its ability to orchestrate the
use of existing Python libraries per
...
[[citep:BLASBasicLinear]] and LAPACK [[citep:LAPACKLinearAlgebra]]--with
...
symbolic-oriented operations--like AD--by way of function tracing
...
of enumerating explicit simplifications for each and every possible model and
...
programming languages is also clear, and, in line with most programming
languages, compilation can be used to improve performance
and--simultaneously--expand the expressive potential of a language's syntax.
...
One noteworthy example involves PyMC's internal logic for determining an
appropriate sampler.  Just like the optimization system in Theano, the graph of
a PyMC model could be manipulated to produce a more suitable, yet equivalent,
model for a given sampler, or--conversely--produce a customized sampler for a
given model.  In extreme cases (an example of which is provide in a later
section), the posterior distribution ultimately estimated by PyMC could be
...
[[citet:WillardRoleSymbolicComputation2017]].  While it is more than possible
to extend the same approach into auto-conjugation and related statistical
optimizations, the scalability and means of specifying new optimizations wasn't
promising.  One element of concern, involved the need use an identity in more than
one direction.  This need can arise when, say, one direction of an identity like
Equation [[eqref:eq:normal-affine-trans]] is useful for computational reasons (e.g.
the funnel problems) and the other direction helps one determine the
distribution type of a term.  The latter is important for determining when a
specialized sampler can be applied, since it's not apparent that \(\mu + \sigma
Y\) in Equation [[eqref:eq:normal-affine-trans]] is a normal random variable unless
the right-to-left rule is applied.

Considerations like these lead quickly into the domain of term rewriting
[[citep:BaaderTermrewritingall1999]].  The example involving Equation
[[eqref:eq:normal-affine-trans]] and the relevance of both directions of its implied
rewrite rules begins to touch upon the ideas of graph normalization/canonicalization,
rewrite rule completion, and general equational reasoning.  Specifically, it would
seem that a convenient normal form would only help solve some problems, and the
ability to apply rewrites in both directions would still be relevant even
after--say--generating a reduced set of rewrite rules using a Knuth-Bendix-like
completion procedure [[citep:Huetcompleteproofcorrectness1981]].

It's likely that classical niceties, like graph normal forms that
guarantee rewrite rule termination, aren't available to us, especially given our
desire for an interactive system for ad-hock additions and experimentation.
However, when they are, we would like the means to apply methods like rewrite
rule completion.
...
GorinovaAutomaticReparameterisationProbabilistic]] are perhaps the most germane;
...
this exact problem in its unit tests, and has to use a generator-based
trampoline to circumvent direct use of the Python stack.
...
it's difficult to asses exactly how expressive their DSL is, or even how well it
...
existing host language.  Also, its "internal" mechanics maintain direct
connections with multiple relevant theoretical concepts (e.g. unification,
complete search, relational programming), so, for--instance--its use
...
[[citep:NearalphaleanTAPDeclarativeTheorem2008]]).  The use of typing rules
to describe the automation of high-level inference algorithms in
[[citet:Waliahighlevelinferencealgorithms2018]] only stresses exactly how close
these fields truly are.
...
It's also possible to address performance and host-language issues--like the
src_scheme[:eval never]{RecursionError}s described above--provided the level
of host integration miniKanren offers.
...
capabilities, it's also possible to send results back to a goal producing a
...
the lack of immediate support for important Scheme/Lisp-like elements.  The most notable
...
matching, they all tend to lack the simplicity afforded by list-based
...
#+ATTR_LATEX: :float nil :placement h!
#+CAPTION: ~CONS~ pair unification and reification using Python's built-in lists.
...
#+ATTR_LATEX: :float nil :placement h!
#+CAPTION: A miniKanren goal that implements the beta-binomial conjugate relation.
...
operations within goals.  One example is could involve the use of
...
identities of the form \(\text{match-form-1} = \text{replace-form-2}\) that
...
forms, it's possible for the rewrite completion algorithms mentioned in Section
[[#sec:want-to-do]] to be applied automatically in some cases, and, if a complete
and reduced rewrite system can be generated from
a src_scheme[:eval never]{conde} it would be worth determining if--or when--the
resulting system could be beneficial to the general performance of miniKanren.
Is it possible that some cases of non-terminating goal orderings could be
avoided by completion?  Would a src_scheme[:eval never]{conde} implied by
completion result in fewer goal evaluations and failed branches?

Alternatively, is it possible that miniKanren could be utilized by the
completion algorithm itself so that it produces potentially relevant results
when a system otherwise wouldn't terminate (e.g. via infinite goal streams)?
...
implementation of experimental completion algorithms)?  Could miniKanren
accelerate research into tensor calculus systems
[[citep:ArrighiOperationalsemanticsformal2004]]?  Proving rewrite termination
and completion itself can involve SAT problems
...
These goals comprise a bulk of the miniKanren work implied here, and their size
could grow quickly over time.  This leads to performance questions that are
possibly answered by work on guided search [[citep:SwordsGuidedSearchminiKanren,
ZhangNeuralGuidedConstraint2018]] and src_scheme[:eval never]{conde} branch
selection [[citep:BoskinSurprisinglyCompetitiveConditional2018]].

There is also little reason to think that a single src_scheme[:eval never]{conde}
will--or even /should/--contain all identities, so a means of compiling
goals and assessing their combined src_scheme[:eval never]{conde} branches
might be worth considering in this context.

Additionally, our Python implementation of miniKanren comes with experimental
support for associative and commutative relations.
...
The process of implementing these relations has opened a few questions that
cannot be properly treated here, instead we would simply like to propose the
general idea for the miniKanren community to consider broadly.
# E.g. how to efficiently handle operators of arbitrary, but known and fixed, arities.
# Goal ordering is an issue, and we've implemented some limited "groundedness" orderings

Finally, we would like to consider the exciting potential for a "feedback loop"
between statistical modeling/machine learning and miniKanren.  If miniKanren can
improve the process of statistical modeling and statistical modeling can improve
the processing of miniKanren [[citep:ZhangNeuralGuidedConstraint2018]], is this
perhaps how mankind can accomplish true AI?
