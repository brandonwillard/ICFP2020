...
Outside of tensor libraries, projects [[citep:JAX2020]] are approaching similar
...
[[#sec:intro-symbolic-computation]] and building upon the well established Python
...
degree to which the framework is expected to integrate with existing offerings,
since we expect this integration to equally apply to existing symbolic
mathematics libraries.

Now, let's consider statistical modeling explicitly.

Statistical modeling surprisingly amenable to symbolic methods--and especially
when one is working within the context of Bayesian models.  Just within the
realm of sample-based model estimation, there are numerous principles and
generalized results that are applicable to large classes of models.
...
[[citep:peasgood_method_2009]] [[citep:roach_meijer_1997]], they are effectively
